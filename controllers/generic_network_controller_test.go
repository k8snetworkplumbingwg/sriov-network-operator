...
var _ = Describe("All Network Controllers", Ordered, func() {
	...
	Context("owner-reference annotations", func() {
		AfterEach(func() {
			k8sClient.DeleteAllOf(ctx, &sriovnetworkv1.SriovNetwork{}, client.InNamespace(testNamespace))
			k8sClient.DeleteAllOf(ctx, &sriovnetworkv1.SriovIBNetwork{}, client.InNamespace(testNamespace))
			k8sClient.DeleteAllOf(ctx, &sriovnetworkv1.OVSNetwork{}, client.InNamespace(testNamespace))
			k8sClient.DeleteAllOf(ctx, &netattdefv1.NetworkAttachmentDefinition{}, client.InNamespace("default"))
		})
		...
		It("applies to new netattachdef", func() {
			...
			Expect(netAttDef.GetAnnotations()).To(HaveKeyWithValue(consts.OwnerAnnotation, "SriovNetwork.sriovnetwork.openshift.io/openshift-sriov-network-operator/sriovnet-blue"))
		})
		...
		It("should migrate existing NetAttachDef to have a value for Owner annotation", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(netAttDef.GetAnnotations()).
					To(HaveKeyWithValue(consts.OwnerAnnotation, "SriovNetwork.sriovnetwork.openshift.io/openshift-sriov-network-operator/netuser"))
