...
// For OpenShift clusters, reads the cluster-wide TLS security profile from the APIServer resource.
// For Kubernetes clusters, reads from environment variables (TLS_CIPHER_SUITES, TLS_MIN_VERSION).
// Empty strings are returned when no configuration is set, which means components use their defaults.
func (r *SriovOperatorConfigReconciler) getTLSTemplateData(ctx context.Context) map[string]string {
	...
	if err != nil {
		log.Log.WithName("getTLSTemplateData").Error(err, "Failed to get TLS config, using defaults")
		return result
	}
	...
	log.Log.WithName("getTLSTemplateData").V(2).Info("TLS config", "TLSCipherSuites", result["TLSCipherSuites"], "TLSMinVersion", result["TLSMinVersion"])
	return result
}
...
func (r *SriovOperatorConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if r.Orchestrator.ClusterType() == consts.ClusterTypeOpenshift && r.isAPIServerCRDAvailable(mgr) {
		log.Log.WithName("SetupWithManager").V(2).Info("Watching for OpenShift APIServer changes to react to TLS profile changes")
...
func (r *SriovOperatorConfigReconciler) syncMetricsExporter(ctx context.Context, dc *sriovnetworkv1.SriovOperatorConfig) error {
	...
	tlsData := r.getTLSTemplateData(ctx)
