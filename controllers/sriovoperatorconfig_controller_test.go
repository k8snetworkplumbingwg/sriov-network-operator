	...
	Context("When is up", func() {
		...
		It("should not remove fields with default values when SriovOperatorConfig is created", func() {
			...
			Consistently(func(g Gomega) {
				...
				g.Expect(obj.Object["spec"]).To(HaveKeyWithValue("enableInjector", false))
				g.Expect(obj.Object["spec"]).To(HaveKeyWithValue("enableOperatorWebhook", false))
				g.Expect(obj.Object["spec"]).To(HaveKeyWithValue("logLevel", int64(0)))
				g.Expect(obj.Object["spec"]).To(HaveKeyWithValue("disableDrain", false))
