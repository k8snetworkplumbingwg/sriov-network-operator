...
func (r *genericNetworkReconciler) handleNetAttDef(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err == nil {
		...
		return []reconcile.Request{{NamespacedName: nadNamespacedName}}
	}

	if !errors.IsNotFound(err) {
		log.Log.WithName(r.controller.Name()+" handleNetAttDef").Error(err, "can't get object", "object", obj.GetNamespace()+"/"+obj.GetName())
		return nil
	}
	...
	if err == nil {
		...
		return []reconcile.Request{{NamespacedName: operatorNamespacedName}}
	}
