	...
	Describe("Generic SriovNetworkNodePolicy", func() {
		...
		Context("Virtual Functions", func() {
			...
			It("should release the VFs once the pod deleted and same VFs can be used by the new created pods", func() {
				...
				By("Checking the sriov resource does not exist in resource request and limit spec")
				_, exist := testPodA.Spec.Containers[0].Resources.Requests[corev1.ResourceName(fmt.Sprintf("openshift.io/%s", resourceName))]
				Expect(exist).To(BeFalse())
				_, exist = testPodA.Spec.Containers[0].Resources.Limits[corev1.ResourceName(fmt.Sprintf("openshift.io/%s", resourceName))]
				Expect(exist).To(BeFalse())
