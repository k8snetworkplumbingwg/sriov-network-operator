...
var _ = Describe("[sriov] Metrics Exporter", Ordered, func() {
	...
	BeforeAll(func() {
		...
		By("Adding ")
		err = addLabelToNamespace(context.Background(), operatorNamespace, "openshift.io/cluster-monitoring", "true")
		Expect(err).ToNot(HaveOccurred())
...
func addLabelToNamespace(ctx context.Context, namespaceName, key, value string) error {
	ns := &corev1.Namespace{}
	err := clients.Get(ctx, runtimeclient.ObjectKey{Name: namespaceName}, ns)
	if err != nil {
