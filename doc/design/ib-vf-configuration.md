...
* Per node IB GUID configuration can be read from a static json file on the host
...
5. Users create pods and request resources from the specific pools
...
Parallel NIC configuration should be supported by synchronising access to the GUID pool.

If there are fewer GUIDs than VFs, then all the GUIDs should be assigned. For the rest of VFs, apply random GUIDs as it's been done before.

If config file doesn't exist in the predefined location, proceed as usual and assign random GUIDs to VFs.

### Config file

Config file can be stored at `/var/opt/infiniband_guids`. This location is writable across most cloud platforms.
...
* `guids` list and range cannot be bot set at the same time for a single device - should return an error
...
1. A script is deployed to the host and creates a static GUID config file
