...
Add a new feature gate `blockDevicePluginUntilConfigured` that, when enabled, adds
an init container to the sriov-device-plugin DaemonSet. This init container blocks
the device plugin main container from starting until the sriov-config-daemon signals
...
1. After successfully applying SR-IOV configuration, the daemon restarts the device
   plugin pod
2. It then waits for the new pod to start and set the `wait-for-config` annotation
3. Once the annotation is detected, the daemon removes it if the desired node state
   contains interface configuration
4. Removing the annotation unblocks the init container, allowing the device plugin
   to start
...
spec:
  featureGates:
    blockDevicePluginUntilConfigured: true
```
...
defined in `pkg/consts/constants.go`.
...
**Upgrade**: When enabling the feature gate:
...
**Downgrade**: When disabling the feature gate:
