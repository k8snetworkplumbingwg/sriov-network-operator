...
func main() {
	...
	// Create status patcher for network controllers (includes embedded condition manager)
	globalStatusPatcher := status.NewPatcher(mgrGlobal.GetClient(), mgrGlobal.GetEventRecorder("sriov-network-operator"), mgrGlobal.GetScheme())
