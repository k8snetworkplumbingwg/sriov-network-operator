...
if ${IS_OS_UBUNTU} ; then
    grub_config="/etc/default/grub.d/10-sriov.cfg"
    # Operate on a copy; create with default content if the drop-in doesn't exist yet
    if [[ ! -f "${chroot_path}${grub_config}" ]]; then
        echo 'GRUB_CMDLINE_LINUX_DEFAULT="${GRUB_CMDLINE_LINUX_DEFAULT}"' > /tmp/grub
    else
