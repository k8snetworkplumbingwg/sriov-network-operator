...
var defaultTestCfg = vars.LogFileSettings{MaxSizeMB: 100, MaxFiles: 5, MaxAgeDays: 30, Compress: false}
...
var _ = g.Describe("File Logging", func() {
	...
	g.It("rotation parameters are correctly set on lumberjack Logger", func() {
		err := initLogWithFileAt(logFile, vars.LogFileSettings{MaxSizeMB: 50, MaxFiles: 3, MaxAgeDays: 7, Compress: true})
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("writes immediately in systemd mode even when InChroot is set", func() {
		savedMode := vars.UsingSystemdMode
		defer func() {
			vars.UsingSystemdMode = savedMode
			vars.InChroot.Store(false)
		}()
		...
		log.Log.Info("systemd-mode-entry")
		...
		case <-time.After(2 * time.Second):
			g.Fail("SyncFileLogger blocked in systemd mode while InChroot was set")
		}

		content, err := os.ReadFile(logFile)
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(string(content)).To(o.ContainSubstring("systemd-mode-entry"))
	})
	...
	g.It("InitLogWithFile rejects path with traversal components", func() {
		err := initLogWithFileAt(tmpDir+"/foo/../escape/config-daemon.log", defaultTestCfg)
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(err.Error()).To(o.ContainSubstring("path traversal"))
	})

	g.It("InitLogWithFile rejects root path", func() {
		err := initLogWithFileAt("/", defaultTestCfg)
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(err.Error()).To(o.ContainSubstring("unsafe root path"))
	})
	...
	g.It("prunes extra backup files immediately when MaxFiles shrinks", func() {
		for day := 1; day <= 5; day++ {
			...
			o.Expect(os.WriteFile(backup, []byte("old"), 0o644)).To(o.Succeed())
		}

		err := initLogWithFileAt(logFile, vars.LogFileSettings{MaxSizeMB: 100, MaxFiles: 2, MaxAgeDays: 30})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		o.Expect(backups).To(o.HaveLen(2))
		o.Expect(backups).To(o.ContainElement("config-daemon-2020-01-05T00-00-00.000.log"))
		o.Expect(backups).To(o.ContainElement("config-daemon-2020-01-04T00-00-00.000.log"))
	})

	g.It("removes files from the previous directory when HostPath changes", func() {
		...
		o.Expect(os.MkdirAll(oldDir, 0o750)).To(o.Succeed())
		o.Expect(os.WriteFile(oldFile, []byte("current"), 0o644)).To(o.Succeed())
		o.Expect(os.WriteFile(filepath.Join(oldDir, "config-daemon-2020-01-01T00-00-00.000.log"), []byte("bak"), 0o644)).To(o.Succeed())

		o.Expect(initLogWithFileAt(oldFile, defaultTestCfg)).To(o.Succeed())
		o.Expect(initLogWithFileAt(newFile, defaultTestCfg)).To(o.Succeed())

		_, err := os.Stat(oldFile)
		o.Expect(os.IsNotExist(err)).To(o.BeTrue())
		_, err = os.Stat(filepath.Join(oldDir, "config-daemon-2020-01-01T00-00-00.000.log"))
		o.Expect(os.IsNotExist(err)).To(o.BeTrue())
	})

	g.It("InitLogWithFile reads configuration from vars", func() {
		savedRoot := vars.FilesystemRoot
		savedCfg := vars.GetLogCfg()
		defer func() {
			vars.FilesystemRoot = savedRoot
			vars.SetLogCfg(savedCfg)
		}()

		vars.FilesystemRoot = tmpDir
		cfg := vars.DefaultLogCfg()
		cfg.HostPath = "/var/log/sriov-init-test"
		vars.SetLogCfg(cfg)

		o.Expect(InitLogWithFile()).To(o.Succeed())

		expected := filepath.Join(tmpDir, "host", "var", "log", "sriov-init-test", LogFileName)
		o.Expect(currentFileWriter()).NotTo(o.BeNil())
		o.Expect(currentFileWriter().lj.Filename).To(o.Equal(expected))

		log.Log.Info("from-vars-cfg")
		SyncFileLogger()
		content, err := os.ReadFile(expected)
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(string(content)).To(o.ContainSubstring("from-vars-cfg"))
	})

	g.It("InitLogWithFile disables file logging when vars.Enabled is false", func() {
		o.Expect(initLogWithFileAt(logFile, defaultTestCfg)).To(o.Succeed())
		o.Expect(currentFileWriter()).NotTo(o.BeNil())
	...
	g.It("InitLogWithFile waits until the chroot window ends", func() {
		savedRoot := vars.FilesystemRoot
		savedCfg := vars.GetLogCfg()
		defer func() {
			vars.FilesystemRoot = savedRoot
		...
		vars.FilesystemRoot = tmpDir
		cfg := vars.DefaultLogCfg()
		cfg.HostPath = "/var/log/sriov-chroot-test"
		vars.SetLogCfg(cfg)
		...
		go func() {
			close(started)
			done <- InitLogWithFile()
		}()
		<-started

		select {
		case err := <-done:
			g.Fail(fmt.Sprintf("InitLogWithFile returned while chroot lock was held: %v", err))
		...
		case err := <-done:
			o.Expect(err).NotTo(o.HaveOccurred())
		case <-time.After(2 * time.Second):
			g.Fail("InitLogWithFile did not return after chroot lock was released")
		}

		expected := filepath.Join(tmpDir, "host", "var", "log", "sriov-chroot-test", LogFileName)
		o.Expect(currentFileWriter()).NotTo(o.BeNil())
		o.Expect(currentFileWriter().lj.Filename).To(o.Equal(expected))
	})
})
