...
var _ = Describe("Aws", func() {
	...
	Context("New and Getters", func() {
		It("should return the host helper", func() {
			Expect(awsPlatform.GetHostHelpers()).To(Equal(hostHelper))
		})
	})
	...
	Context("Init", func() {
		...
		It("should create devices from metadata if node state is nil", func() {
			...
			hostHelper.EXPECT().GetInterfaces().Return([]sriovnetworkv1.InterfaceExt{{
	...
	Context("Plugin Management", func() {
		It("should return a virtual plugin", func() {
			main, additionals, err := awsPlatform.GetPlugins(nil)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should return an error for systemd plugin request", func() {
			_, err := awsPlatform.SystemdGetPlugin("any-phase")
			Expect(err).To(MatchError("aws platform not supported in systemd"))
		})
	})
	...
	Context("Bridge Discovery", func() {
		It("should return an error if ManageSoftwareBridges is true", func() {
			...
			Expect(err).To(MatchError("bridge configuration not supported on openstack platform"))
		})
